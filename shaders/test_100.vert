...
void	main()
{
    ...
    {
        float   perturbation = cos( tmp_vertex.x + u_advance * 3.14 ) + sin( tmp_vertex.y + u_advance * 3.14 );

        tmp_vertex = tmp_vertex + vec3(0.5, 0.5, 0.5) * perturbation - vec3(-0.25, -0.25, -0.25);

        tmp_normal = tmp_normal - vec3(0.0, 0.3, 0.6) * perturbation;
        normalize( tmp_normal );

        tmp_color = tmp_normal - vec3(1.0, 1.0, 1.0) * perturbation;
        normalize( tmp_color );
    }
